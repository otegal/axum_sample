# Development
FROM rust:1.55.0-slim-buster AS dev
...
RUN cargo build --color never \
  && rm src/*.rs \
  && find $CARGO_TARGET_DIR/ -name "libaxum_sample*" -delete \
  && find $CARGO_TARGET_DIR/ -name "axum_sample*" -prune -exec rm -rf {} +
...
COPY --from=build /tmp/target/release/axum_sample /app/bin

CMD ["/app/bin/axum_sample"]